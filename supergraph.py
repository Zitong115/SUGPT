...
class Supergraph(object):
    ...
    def FindCandidateOfOneVertices(self, vertex, escape = 0, vertices_pool = None, influenced_tree2sv = None):
        ...
        for k,i in enumerate(ordered_tree):
            ...
            # record the tree
            # self.supernode[vertex].addUsedTree(i)
    ...
    def DeleteOneEdge(self, di, dj, directed = False):
        ...
        #        influenced_set.add(self.GetGroupid(treeno = old_treeno_i, groupno=self.graph.trees[old_treeno_i].getGroupno(dj_place)))
    ...
    def ReviseSns(self, dismissed_vertices, influenced_sv_set = None, uninfluenced_sv_set = None):
        ...
        for sv in list(influenced_sv_set) :
            ...
            if(sv < 0):
                ...
                if(newkey in self.supernode.keys()): # not added
        ...
        print("737 len(self.supernode):", len(self.supernode))

        # influenced_sv_set -= changed_sv_set

        # adjust supernode keys step 2
        """
        ...
        for sv in self.supernode:
            if(sv < 0):
                ...
                del self.supernode_copy[sv] # remove the old supernode key
    ...
    def UpdateGroup(self, edgelist, deleted_edge_idx, directed = False, influenced_groups = None, implement = False):
        ...
        print("len(influenced_groups): %d" % len(influenced_groups))
        return influenced_groups
    ...
    def printDebugInfo(self):
        ...
        if(6461 in self.supernode):
            print("supernode 6461 vertice:", self.supernode[6461].vertices)
        else:
            print("6461 not in self.supernode")

        if(6461 in self.supernode):
            print("supernode 7046 vertice:", self.supernode[7046].vertices)
        else:
            print("7046 not in self.supernode")
    ...
    def EdgeDeleteGSS(self, deleted_edge_idx):
        
        edgelist= list(self.graph.G.edges)
        
        print("Remove %d edges in total." % len(deleted_edge_idx))
        ...
        for deleted_edge in deleted_edge_idx:
            ...
            src, dst = str(edgelist[deleted_edge][0]), str(edgelist[deleted_edge][1])
        ...
        for sekey in tmpsekeys:
            h_list = sekey.split(edge_connector)
            self.superedge.remove([int(h_list[0]), int(h_list[1])])
            # self.superedge_weight.pop(sekey)
            cnt += 1
        """        
        t3 = time.time()
    ...
    def EdgeDelete(self, deleted_edge_idx, removed_vertices = []):
        ...
        print("len(uninfluenced_sv_set): ", len(uninfluenced_sv_set), "3134 / 8214 in influenced_sv_set: ", 3134 in influenced_sv_set, 8214 in influenced_sv_set)
